...
CREATE TABLE IF NOT EXISTS fantasy_payouts (
  ...
  prize_transfer_status TEXT NOT NULL DEFAULT 'pending'
    CHECK (prize_transfer_status IN ('pending', 'confirmed', 'failed')),
  transfer_retry_count INT NOT NULL DEFAULT 0,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  UNIQUE (game_id, telegram_id)
);

ALTER TABLE IF EXISTS fantasy_payouts
